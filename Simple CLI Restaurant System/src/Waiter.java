...
public class Waiter {
	...
	public void printWaiterOptions() {
		...
		System.out.println("2. View Customers Info ("+Customer.customerList.size()+" customer(s) available)");
		...
		if (option == 1) {
			...
			if (Food.getFoodServingQueue().size()==0) {
				...
				System.out.println("Please select food to serve (Enter 0 to exit): ");
	...
	private void createCustomer() {
		System.out.println("Please Enter Customer username/tableNo: ");
		Scanner scanner = new Scanner(System.in);
	...
	public void printFoodQueue() {
		...
		for (int i = 0 ; i < Food.foodServingQueue.size();i++) {
			System.out.println(i+1+". Item: "+Food.foodServingQueue.get(i).getName()+"  Quantity: "+Food.foodServingQueue.get(i).getQuantity()+"  Username/TableNo: "+ Food.foodServingQueue.get(i).getCustomer().getUserName());
		}
		System.out.println("-------------------------------------------------------------------------");
	}
}
